...
// Get bank details for a school
exports.getBankDetails = async (req, res) => {
  try {
    const { schoolId } = req.params;
    
    const school = await School.findById(schoolId).select('bankDetails schoolName');
    ...
    // Ensure we have a proper bankDetails object with all required fields
    const bankDetails = {
      bankName: school.bankDetails?.bankName || 'Not Available',
      accountNumber: school.bankDetails?.accountNumber || school.bankDetails?.accountNo || 'Not Available',
      ifscCode: school.bankDetails?.ifscCode || school.bankDetails?.ifsc || 'Not Available',
      branch: school.bankDetails?.branch || school.bankDetails?.branchName || 'Not Available',
      accountHolderName: school.bankDetails?.accountHolderName || school.bankDetails?.accountName || 'School Account'
    };
...
exports.updateBankDetails = async (req, res) => {
  try {
    ...
    school.bankDetails = {
      ...
      branch: branch ? branch.trim() : school.bankDetails?.branch,
      accountHolderName: accountHolderName ? accountHolderName.trim() : school.bankDetails?.accountHolderName
    };

    await school.save();

    // Sync the updated bank details to the school's database if it exists
    if (school.databaseCreated) {
      try {
        await syncSchoolInfoToDatabase(school);
      } catch (syncError) {
        console.error('Error syncing bank details to school database:', syncError);
